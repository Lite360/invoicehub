...
export default function QuotationForm() {
  ...
  // Form state
  const [quotationNumber, setQuotationNumber] = useState(genQuotationNumber());
  ...
  const handleSave = async (status = 'draft') => {
    setSaving(true);
    const payload = {
      quotationNumber, clientName, clientEmail, clientPhone, clientAddress,
      issueDate, validUntil, currency, taxRate, taxAmount,
      discountType, discountValue, discountAmount,
      subtotal, total, notes, terms, status,
      items: items.map(i => ({
        description: i.description,
        quantity: i.quantity,
    ...
    try {
      ...
      navigate(`/app/quotations/${saved.id}`);
    } catch (err) {
